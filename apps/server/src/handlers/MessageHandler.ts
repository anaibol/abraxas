import { type ClientMessages, ClientMessageType, ServerMessageType, MathUtils, VOICE_RANGE } from "@abraxas/shared";
...
export class MessageHandler {
	...
	registerHandlers(
		...
		register(ClientMessageType.GMTeleport, (c, m) => MovementHandlers.handleGMTeleport(this.ctx, c, m));
		...
		register(ClientMessageType.QuestComplete, (c, m) => InteractionHandlers.handleQuestComplete(this.ctx, c, m));

		register(ClientMessageType.Chat, (c, m) => SocialHandlers.handleChat(this.ctx, c, m));
		register(ClientMessageType.GroupInvite, (c, m) => SocialHandlers.handleGroupInvite(this.ctx, c, m));
		register(ClientMessageType.GroupAccept, (c, m) => SocialHandlers.handleGroupAccept(this.ctx, c, m));
		...
		register(ClientMessageType.TradeAccept, (c, m) => EconomyHandlers.handleTradeAccept(this.ctx, c, m));
		register(ClientMessageType.TradeOfferUpdate, (c, m) => EconomyHandlers.handleTradeOfferUpdate(this.ctx, c, m));
		register(ClientMessageType.TradeConfirm, (c) => EconomyHandlers.handleTradeConfirm(this.ctx, c));
		register(ClientMessageType.TradeCancel, (c) => EconomyHandlers.handleTradeCancel(this.ctx, c));
		register(ClientMessageType.BankDeposit, (c, m) => EconomyHandlers.handleBankDeposit(this.ctx, c, m));
		register(ClientMessageType.BankWithdraw, (c, m) => EconomyHandlers.handleBankWithdraw(this.ctx, c, m));
