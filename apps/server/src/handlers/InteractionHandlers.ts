import { type ClientMessages, ClientMessageType, ServerMessageType, MERCHANT_INVENTORY, QUESTS, MathUtils } from "@abraxas/shared";
...
export class InteractionHandlers {
	...
	static openDialogue(ctx: RoomContext, client: Client, player: Player, npc: Npc) {
		const dialogue = ctx.systems.quests.getDialogueOptions(
			player.dbId,
			npc.sessionId,
			npc.type,
		);
		...
		ctx.systems.quests
			...
			.then((questDef) => {
				if (questDef) {
					...
					const state = ctx.systems.quests.getQuestState(
						player.dbId,
						data.questId,
					);
					if (state) {
