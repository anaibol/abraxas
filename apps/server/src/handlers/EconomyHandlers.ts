import { type ClientMessages, ClientMessageType, ServerMessageType, ITEMS, type TradeState } from "@abraxas/shared";
import type { Client } from "@colyseus/core";
import type { RoomContext } from "./RoomContext";
import type { Player } from "../schema/Player";
import { HandlerUtils } from "./HandlerUtils";
import { InteractionHandlers } from "./InteractionHandlers";
import { MERCHANT_INVENTORY } from "@abraxas/shared";
...
export class EconomyHandlers {
	...
	static handleTradeConfirm(ctx: RoomContext, client: Client): void {
		...
		if (trade) {
			...
			if (ctx.systems.trade.canComplete(trade)) {
				ctx.systems.trade
					.executeTrade(trade, ctx.state.players)
					.then((success) => {
						if (success) {
							EconomyHandlers.sendToParticipants(
								ctx,
								trade,
	...
	static handleBuyItem(
		...
		if (
			ctx.systems.inventory.addItem(player, data.itemId, quantity, (msg) =>
	...
	static handleSellItem(
		...
		if (ctx.systems.inventory.removeItem(player, data.itemId, quantity)) {
